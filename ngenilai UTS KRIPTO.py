...
def menu_dekripsi_key_relations():
    ciphertext = "KSMEHZBBLKSMEMPOGAJXSEJCSFLZSY"
    key = "RELATIONS"

    cipher_pairs = [ciphertext[i:i+2] for i in range(0, len(ciphertext), 2)]
    repeated_key = (key * ((len(cipher_pairs) * 2) // len(key) + 1))[:len(cipher_pairs) * 2]

    print(f"\n🧩 Ciphertext: {ciphertext}")
    print(f"🔑 Key: {key}")
    print(f"📦 Jumlah pasangan cipher: {len(cipher_pairs)}\n")

    full_plaintext = ""

    for n, cipher_pair in enumerate(cipher_pairs):
        key_pair = repeated_key[n*2:n*2+2]
        decrypted_pair = vigenere_decrypt_pair(cipher_pair, key_pair)
        full_plaintext += decrypted_pair

        print(f"n = {n + 1}")
        for i in range(2):
            c = cipher_pair[i]
            k = key_pair[i]
            p_val = (ord(c) - ord(k)) % 26
            p_char = chr(p_val + 65)
            print(f"Cipher: {c}")
            print(f"Key:    {k}")
            print(f"Plain: ({ord(c)} - {ord(k)}) mod 26 = {p_val}")
            print(f"Plain:  {p_char}\n")
        print("=====================================")

    print(f"\n🔓 Hasil Plaintext Lengkap: {full_plaintext}")
